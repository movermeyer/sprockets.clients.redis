...
class ShardedRedisConnection(object):
    ...
    .. code::
        ...
        >>> shard = RedisShard()
        >>> shard
        <hostname=localhost, servers="127.0.0.1">
        >>> shard.set('foo', 1)
        >>> shard.get('foo')
        '1'
        >>> shard.delete('foo')
    ...
    def info(self):
        """Return the health of each redis server we're connected to."""
        stats = {
        ...
        db = self.config.db or 'db0'

        LOGGER.info('Getting Redis health for db "%s"', db)
        for host, connection in self._connections.items():
            LOGGER.debug('Getting Redis health for host "%s"', host)
            info = connection.info()
            stats['commands_processed'] += info['total_commands_processed']
            stats['memory_used'] += info['used_memory']
            stats['keys'] += info.get(db, {}).get('keys', 0)
            stats['expires'] += info.get(db, {}).get('expires', 0)
            stats['average_ttl'] += info.get(db, {}).get('avg_ttl', 0)

        if len(self._connections.items()) > 1:
            stats['average_ttl'] /= float(len(self._connections.items()))
